...
public class CartItemController {
    ...
    public void addCartItem(@PathVariable(value = "productId") int productId) {
        // here, we don't add method, then this will deal with all method(post,get....)
        ...
        for (int i = 0; i < cartItems.size(); i++) {
            //check if item already in cart; if in it, quality + 1; else add
            CartItem cartItem = cartItems.get(i);
            if (product.getId() == (cartItem.getProduct().getId())) {
                cartItem.setQuantity(cartItem.getQuantity() + 1);
                cartItem.setPrice(cartItem.getQuantity() * cartItem.getProduct().getProductPrice());
                cartItemService.addCartItem(cartItem);
                // here, addCartItem could also update
                return;
            }
        }

        // if no duplicate item
