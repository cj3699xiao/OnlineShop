...
public class CartController {
    ...
    public ModelAndView getCartId(){
        ...
        //in documentary, if we use Security frame
        //SecurityContextHolder will hold Authentication information
        Authentication loggedInUser = SecurityContextHolder.getContext().getAuthentication();
        String username = loggedInUser.getName();
        Customer customer = customerService.getCustomerByUserName(username);
        modelAndView.addObject("cartId", customer.getCart().getId());
        // we give cartId back to user, if user do more operation, we could use it
        return modelAndView;
    }
